...
txt_folder_path = '/mnt/data/CCM/snndatabase/snncodingdata_new_train/'
txt_file_path = findfile(txt_folder_path, '.txt')
print(txt_file_path)

norm_data_list = []
for txt_file in txt_file_path:
    data_per_file = [[] for i in range(4000)]
...
for i in range(len(norm_data_list)):
    ...
    sensor_indices = [[] for i in range(4000)]
    spike_times_list = [[] for i in range(4000)]
...
# ORN
neuron_groups['AN'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['AN'].tau = 100 * ms
# neuron_groups['AN'].tau = 50 * ms

neuron_groups['SON'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['SON'].tau = 800 * ms
# neuron_groups['SON'].tau = 50 * ms

neuron_groups['II'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['II'].tau = 800 * ms
# neuron_groups['II'].tau = 50 * ms

neuron_groups['IC'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['IC'].tau = 800 * ms
# neuron_groups['IC'].tau = 50 * ms

neuron_groups['THA'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['THA'].tau = 800 * ms
# neuron_groups['THA'].tau = 50 * ms

neuron_groups['AC_CORE'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['AC_CORE'].tau = 800 * ms
# neuron_groups['AC_CORE'].tau = 50 * ms

neuron_groups['AC_BELT'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['AC_BELT'].tau = 800 * ms
# neuron_groups['AC_BELT'].tau = 50 * ms

neuron_groups['AC_PARA'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['AC_PARA'].tau = 800 * ms
# neuron_groups['AC_PARA'].tau = 50 * ms

neuron_groups['OFC'] = NeuronGroup(sensor_numbers, eqs1, threshold='v>1', reset='v = 0', method='euler')
neuron_groups['OFC'].tau = 800 * ms
# neuron_groups['OFC'].tau = 50 * ms


weight_path = '/mnt/data/CCM/snndatabase/weights_new_1/'
starting = 'sound_'
ending = '_1'
...
weightMatrix_S0 = get_matrix_from_file(weight_path + starting + 'SI' + ending + '.npy')
...
weightMatrix_S2 = get_matrix_from_file(weight_path + starting + 'S1' + ending + '.npy')
...
weightMatrix_S3 = get_matrix_from_file(weight_path + starting + 'S2' + ending + '.npy')
...
weightMatrix_S4 = get_matrix_from_file(weight_path + starting + 'S3' + ending + '.npy')
...
weightMatrix_S5 = get_matrix_from_file(weight_path + starting + 'S4' + ending + '.npy')
...
weightMatrix_S6 = get_matrix_from_file(weight_path + starting + 'S5' + ending + '.npy')
...
weightMatrix_S7 = get_matrix_from_file(weight_path + starting + 'S6' + ending + '.npy')
...
weightMatrix_S8 = get_matrix_from_file(weight_path + starting + 'S7' + ending + '.npy')
...
weightMatrix_S9 = get_matrix_from_file(weight_path + starting + 'S8' + ending + '.npy')
connections['S8'].w = weightMatrix_S9[connections['S8'].i, connections['S8'].j]

connections['S9'] = Synapses(neuron_groups['AC_PARA'], neuron_groups['OFC'], model=eqs_stdp, on_pre=eqs_stdp_pre,
                             on_post=eqs_stdp_post, method='exact')
connections['S9'].connect(condition='abs(i-j)<4 and i!=j')
weightMatrix_S10 = get_matrix_from_file(weight_path + starting + 'S9' + ending + '.npy')
...
weightMatrix_S11 = get_matrix_from_file(weight_path + starting + 'S10' + ending + '.npy')
...
weightMatrix_S12 = get_matrix_from_file(weight_path + starting + 'S11' + ending + '.npy')
connections['S11'].w = weightMatrix_S12[connections['S11'].i, connections['S11'].j]

connections['S12'] = Synapses(neuron_groups['AC_BELT'], neuron_groups['OFC'], model=eqs_stdp, on_pre=eqs_stdp_pre,
                             on_post=eqs_stdp_post, method='exact')
connections['S12'].connect(condition='abs(i-j)<4 and i!=j')
weightMatrix_S13 = get_matrix_from_file(weight_path + starting + 'S12' + ending + '.npy')
...
for neuron_indices, neuron_spike_times in zip(sensor_indices_copy_list, input_spike_times_list):
    ...
    net.run(run_time)
    new_txt_file = open('/mnt/data/CCM/snndatabase/aftercoding_new/%s.txt' %
                        (txt_file_path[num_examples].split('/')[-2].split('.t')[0]+'-'+txt_file_path[num_examples].split('/')[-1].split('.t')[0]), 'w', encoding='utf-8')
    MC_spike_monitors = [spike_monitors['OFC0'], spike_monitors['OFC1'],spike_monitors['OFC2'],spike_monitors['OFC3'],spike_monitors['OFC4']]
