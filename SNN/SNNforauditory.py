...
def timetobinary(timearray, T):
    binscal=4001/T
    x = np.asarray(timearray/ binscal, dtype=int)
    spike_array = np.zeros(int(4001/ binscal), dtype=bool)
    for i in x:
        spike_array[i] = True
    return spike_array
...
# 数据集路径
train_dir = '/mnt/data/CCM/snndatabase/snncodingdata_new/'
# train_dir = '/mnt/data/CCM/snndatabase/CQT_Encoding/'
# train_dir='/mnt/data/CCM/snndatabase/snncodingdata_36N_4s_rename/'
# train_dir='/mnt/data/CCM/snndatabase/fusion2/'
# train_dir = '/mnt/data/CCM/snndatabase/snncodingdata_18N_5s/'
# 每批数据样本量，根据显存大小设置
batch_size = 64
# 学习率
learning_rate = 1e-2
