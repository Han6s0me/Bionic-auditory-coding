# -*- coding: utf-8 -*-
"""
Created on Tue Sep 26 20:36:13 2023
...
filename = '/mnt/data/CCM/snndatabase/TID_DATA_8k/1/1.wav'
# src, sr = librosa.core.load(filename, sr=8000)
# filename = r"D:\浙大项目\四模态机器人\testsound\录音\7.wav"
waveform, sample_rate = torchaudio.load(filename)
waveform=np.array(waveform)
waveform=waveform.reshape(-1)

outdata_1=add_noise(waveform,0,8000)
outdata_2=awgn(waveform,0)
segments = VAD(waveform, 8000)
if len(segments)!=0:
    outdata_3=waveform[segments[0][0]:segments[0][1]]
plt.figure()
plt.plot(waveform,'k')
plt.show()
plt.figure()
plt.plot(outdata_2,'r')
plt.show()
plt.figure()
plt.plot(outdata_3,'b')
plt.show()
...
def read_origindata(dir):#读取初始编码
    ...
    for folder in os.listdir(dir):
        ...
        for file in file_path:
            ...
            # sounddata=add_noise(sounddata,10,8000)
            ...
            # # f, t, Sxx = signal.spectrogram(sounddata, sample_rate)
            # # Engry=10*np.log10(Sxx)
            # cqtpec = cqt(sounddata, sr=sample_rate,fmin=32,n_bins=83,bins_per_octave=12,hop_length=96,pad_mode='wrap')
            # Engry=np.abs(10*np.log10(cqtpec))
...
torch.save(net,'./snnmodel/ccm_snn_model_TID_cqt_possion_T2_1.pth')
...
net = torch.load('./ccm_snn_model_TID8k.pth',map_location='cpu')
# filename = r"D:\浙大项目\SNN\Database\DATA\DATA_WAV\7_seven_wav\Bro004-me013-c2-u1-6-2028-7.wav"
filename = r"D:\浙大项目\四模态机器人\testsound\录音\changed_9_mou.wav"
waveform, sample_rate = torchaudio.load(filename)
...
net = torch.load('./snnmodel/ccm_snn_model_TID_cqt_possion_T2_1.pth',map_location='cpu')
...
import numpy as np
#加载语音文件
song = AudioSegment.from_file(r'C:\Users\CCM\iCloudDrive\Documents\新录音 42.m4a')#路径可以使用绝对路径也可以使用相对路径
song.export(r'C:\Users\CCM\iCloudDrive\Documents\changed_9.wav','wav')#**代表转换后的wave文件名
# data = wave.open("changed_0.wav")
